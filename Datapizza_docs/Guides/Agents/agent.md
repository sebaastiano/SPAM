...
Source: http://docs.datapizza.ai/0.0.9/Guides/Agents/agent/
...
The above agent is quite basic, so let's make it more functional by adding tools.
...
`run(task_input: str, tool_choice = "auto", **kwargs) -> str` Execute a task and return the final result.
...
Stream the agent's execution process, yielding intermediate steps. (Does not stream the single answer)
...
`a_run(task_input: str, **kwargs) -> str` Async version of run.
...
`a_stream_invoke(task_input: str, **kwargs) -> AsyncGenerator[str | StepResult, None]` Stream the agent's execution process, yielding intermediate steps. (Does not stream the single answer)
...
An agent can call another one using `can_call` method:
...
Alternatively, you can define a tool that manually calls the agent. The two solutions are more or less identical.
...
agent = Agent(
    ...
    stream=True, # With stream=True, the agent will stream the client response, not only the intermediate steps
)
